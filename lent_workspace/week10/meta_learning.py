...
class A2C(nn.Module):
    def __init__(self, input_size, n_actions, bandits, hidden_size=48, num_layers=1, gamma = 0.75):
        ...
        self.rnn = CB_GRU(input_size, self.hidden_size)
        ...
        self.rnn.v_t = (torch.zeros(self.x.size(0), self.hidden_size, dtype=torch.float32, device=device))

        self.hiddens = [self.rnn.v_t]
        self.xs = []
    ...
    def forward(self,x):
        lstm_output = self.rnn(x)
        self.hiddens.append(self.rnn.v_t)
        ...
        while self.episode < (TOTAL_EPISODES):
            ...
            while not done:
                ...
                if t_step % T_MAX == 0 and t_step != 0: 
                    ...
                    self.actor_critic.rnn.v_t = self.actor_critic.rnn.v_t.detach()
                    self.actor_critic.rnn.v_t = (torch.zeros(self.actor_critic.x.size(0), self.actor_critic.hidden_size, dtype=torch.float32, device=device))
