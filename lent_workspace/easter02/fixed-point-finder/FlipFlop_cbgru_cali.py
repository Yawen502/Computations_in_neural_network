...
class multiscale_RNN_cell(nn.Module):
	def __init__(self, input_size, hidden_size):
		...
		for W in [self.W, self.P, self.K, self.P_z]:
			glorot_init(W)
		nn.init.constant_(self.b_z, 0.0)
	...
	def forward(self, x):        
		...
		self.z_t = self.z_low + (self.z_high - self.z_low)*self.Sigmoid(torch.matmul(self.K , self.r_t) + torch.matmul(self.P_z, x) + self.b_z)
...
class FlipFlop(nn.Module):
	...
	def train(self, train_data_gen, valid_data_gen, 
		...
		np.save('lent_workspace\\easter02\\fixed-point-finder\\cali_cbgru_mse_errors.npy', mse_errors)
