...
class multiscale_RNN_cell(nn.Module):
	...
	def forward(self, x):        
		...
		self.r_t = (1 - self.z_t) * self.r_t + self.z_t * (torch.matmul(self.W, self.Sigmoid(self.r_t) + torch.matmul(P, x) + self.b_v))
		self.r_t = torch.transpose(self.r_t, 0, 1)                                 
...
class FlipFlop(nn.Module):
	...
	def train(self, train_data_gen, valid_data_gen, 
		...
		np.save('lent_workspace\\easter02\\fixed-point-finder\\cali_multiscale_mse_errors.npy', mse_errors)
