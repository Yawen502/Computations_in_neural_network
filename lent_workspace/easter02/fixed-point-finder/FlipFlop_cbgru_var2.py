...
class multiscale_RNN_cell(nn.Module):
	...
	def forward(self, x):        
		...
		self.z_t = self.z_low + (self.z_high - self.z_low)*self.Sigmoid(torch.matmul(self.K, self.v_t)+torch.matmul(self.P_z, x) +self.b_z)
		self.v_t = (1 - self.z_t) * self.v_t + self.z_t*self.Sigmoid(torch.matmul(self.W, self.v_t) + torch.matmul(P, x) + self.b_v)
		self.v_t = torch.transpose(self.v_t, 0, 1)        
...
class FlipFlop(nn.Module):
	...
	def train(self, train_data_gen, valid_data_gen, 
		...
		np.save('lent_workspace\\easter02\\fixed-point-finder\\cbgru_var2_mse_errors.npy', mse_errors)
