...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Remove hidden states to test working memory
        reset_step = sequence_length * reset_proportion

        self.lstm.rnncell.r_t = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        for step in range(x.size(1)):
            x_step = x[:,step,:]
            out_step = self.lstm.rnncell(x_step)
